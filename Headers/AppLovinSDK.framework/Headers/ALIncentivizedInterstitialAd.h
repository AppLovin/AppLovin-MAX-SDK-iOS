...
/**
 * This class shows rewarded videos to the user. These differ from regular interstitials in that they allow you to provide your user virtual
 * currency in exchange for watching a video.
 */
 ...
 *
 * Invoke this once to pre-load, then do not invoke it again until the ad has has been closed (e.g., in {@link ALAdDisplayDelegate}'s `adWasHidden`
 * callback).
 *
 * @warning You may pass a `nil` argument to `preloadAndNotify` if you intend to use the synchronous ({@link readyForDisplay}) flow. Note that this is
 * <em>not</em> recommended; AppLovin <em>highly recommends</em> that you use an ad load delegate.
 * 
...
/**
 * Checks if an ad is currently ready on this object. You must first have called `preloadAndNotify` in order for the value returned by this method to be
 * meaningful.
 *
 * @warning It is highly recommended that you implement an asynchronous flow (using an {@link ALAdLoadDelegate} with `preloadAndNotify`) rather than
 * checking this property. This class does not contain a queue and can hold only one preloaded ad at a time. Therefore, you should <em>not</em> simply call
 * `preloadAndNotify`: any time this method returns `NO`; it is important to invoke only one ad load — then not invoke any further loads until the ad has been
 * closed (e.g., in {@link ALAdDisplayDelegate}'s `adWasHidden` callback).
 *
 ...
 *
 * You must have called `preloadAndNotify` before you call `show`.
 */
+ (void)show;
 ...
 * Shows an incentivized interstitial over the current key window, by using the most recently pre-loaded ad.
 *
 * You must have called `preloadAndNotify` before you call `showAndNotify`.
 *
...
/**
 * Checks if an ad is currently ready on this object. You must call preloadAndNotify in order to reach this state.
 *
 * @warning It is highly recommended that you implement an asynchronous flow (using an {@link ALAdLoadDelegate} with `preloadAndNotify`) rather than
 * checking this property. This class does not contain a queue and can hold only one preloaded ad at a time. Therefore, you should <em>not</em> simply call
 * `preloadAndNotify`: any time this method returns `NO`; it is important to invoke only one ad load — then not invoke any further loads until the ad has been
 ...
 *
 * You must have called `preloadAndNotify` before you call `showAd`.
 *
 * By using the {@link ALAdRewardDelegate}, you can verify with AppLovin servers that the video view is legitimate, as AppLovin will confirm whether the
 * specific ad was actually served. Then AppLovin will ping your server with a URL at which you can update the user's balance. The Reward Validation Delegate
 * will tell you whether this service was able to reach AppLovin servers or not. If you receive a successful response, you should refresh the user's balance
