...
{
    NSLog(@"Redirecting from cta button click");
...
{
    NSLog(@"Redirecting from card click");
    [self handleClickForAd: self.ad];
}
...
- (void)refresh
{
    NSLog(@"----------Begin refreshing carousel card view----------");
    ...
    if ( ad )
    {
        NSLog(@"Refreshing ad (%@) for card view", ad.adIdNumber);
        ...
        {
            NSLog(@"Native ad (%@) is pre-cached. Refreshing image resources", ad.adIdNumber);
...
#pragma mark - Populate with appropriate stars asset depending on starRating
    ...
    {
        NSLog(@"Refreshing nil native ad for card view. Clearing view...");
        
        [self clearView];
        [self al_hideActivityIndicatorAnimated: YES];
    }
    
    NSLog(@"----------Finish refreshing carousel card view----------");
}
...
- (void)handleClickForAd:(ALNativeAd *)ad
{
    ...
    else
    {
        ...
        NSLog(@"Attempting to open CTA URL with a nil native ad");
    }
}
...
- (void)trackImpression
{
    ...
    {
        NSLog(@"Handling displaying of native ad (%@)", self.ad.adIdNumber);
        
        if ( !self.cardState.impressionTracked )
        {
            self.cardState.impressionTracked = YES;
            NSLog(@"Tracking impression for ad (%@)", self.ad.adIdNumber);
            [self.ad trackImpression];
        }
    }
    else
    {
        NSLog(@"Attempting to handle a nil slot or nil card state being displayed");
    }
}
