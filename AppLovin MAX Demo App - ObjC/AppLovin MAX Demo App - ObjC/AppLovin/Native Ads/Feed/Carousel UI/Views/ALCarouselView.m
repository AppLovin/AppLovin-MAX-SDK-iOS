...
- (void)nativeAdService:(ALNativeAdService *)service didLoadAds:(NSArray *)ads
{
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        {
            NSLog(@"Unable to notify native ad load delegate because of exception: %@", exception);
        }
    }];
}
...
{
    NSLog(@"Native ad service did fail to load native ad with error: %ld", code);
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        @catch (NSException *exception)
        {
            NSLog(@"Unable to notify native ad load delegate about failing to load because of exception: %@", exception);
        }
    }];
}
...
- (void)renderAdsIfNeeded
{
    ...
    {
        NSLog(@"Attempting to re-render native ad(s)");
    }
    else
    {
...
- (void)refreshView
{
    NSLog(@"Begin refreshing carousel view for number of native ads: %lu", self.nativeAds.count);
    ...
    {
        NSLog(@"Begin refreshing for card at index %ld", cardIndex);
        ...
        {
            NSLog(@"Refreshing card view at index: %ld with slot at index: %ld", cardIndex, adIndex);
            ...
            {
                NSLog(@"Card at index: %ld currently has its images pre-cached", cardIndex);
            ...
            {
                NSLog(@"Card at index: %ld is not pre-cached", cardIndex);
                ...
                {
                    NSLog(@"Card at index: %ld is already currently pre-caching", cardIndex);
                }
                else
                {
                    cardState.precaching = YES;
                    
                    NSLog(@"Begin pre-caching for card at index: %ld", cardIndex);
        ...
        {
            NSLog(@"Hiding card at card index: %ld", cardIndex);
            ...
            {
                NSLog(@"Hiding middle card because of nil ad.");
            }
        }
    }
    ...
    NSLog(@"Finish refreshing carousel view");
}
...
- (void)setNativeAds:(NSArray * __nullable)nativeAds
{
    if ( self.nativeAds.count > 0 )
    {
        ...
        for ( id obj in self.nativeAds )
        {
            if ( ![obj isKindOfClass: [ALNativeAd class]] )
            {
                ...
                NSLog(@"Found an object of invalid type (%@) in nativeAds", NSStringFromClass([obj class]));
    ...
    else
    {
        ...
        NSLog(@"Setting native ads of count 0. Clearing view...");
        [self clearView];
    }
...
- (void)setCurrentAdIndex:(NSInteger)currentAdIndex
{
    if ( currentAdIndex < self.nativeAds.count )
    {
        ...
        {
            NSLog(@"Setting same current ad index of %ld", currentAdIndex);
        }
        else
        {
            NSLog(@"Setting new current ad index of %ld", currentAdIndex);
    ...
    {
        NSLog(@"Setting out-of-bounds index of %ld", currentAdIndex);
    }
}
...
{
    NSLog(@"Finished pre-caching images for slot (%@). Rendering...", ad.adIdNumber);
    ...
    {
        NSLog(@"Finished pre-caching images for ad (%@). Card is not on screen, stashing...", ad.adIdNumber);
    }
}
...
- (void)nativeAdService:(ALNativeAdService *)service didPrecacheVideoForAd:(ALNativeAd *)ad
{
    if ( ad.videoURL )
    {
        ...
        {
            NSLog(@"Finished pre-caching for slot (%@) with valid video in active card", ad.adIdNumber);
        ...
        {
            NSLog(@"Finished pre-caching video for ad (%@). Card is not on screen, stashing...", ad.adIdNumber);
        }
    }
    else
    {
        // Finished pre-caching for slot without video
        NSLog(@"Finished pre-caching for ad (%@) without video", ad.adIdNumber);
    }
...
- (void)nativeAdService:(ALNativeAdService *)service didFailToPrecacheImagesForAd:(ALNativeAd *)ad withError:(NSInteger)errorCode
{
    ...
    NSLog(@"Failed to precache images for ad (%@) with error code: %ld", ad.adIdNumber, errorCode);
}
...
- (void)nativeAdService:(ALNativeAdService *)service didFailToPrecacheVideoForAd:(ALNativeAd *)ad withError:(NSInteger)errorCode
{
    ...
    NSLog(@"Failed to precache video for ad (%@) with error code: %ld", ad.adIdNumber, errorCode);
}
