...
- (void)refresh
{
    ...
    if ( ad )
    {
        ...
        NSLog(@"Begin refresh media view for slot ID: %@ %@", ad.adIdNumber, ad.title);
        ...
        if ( self.cardState.currentlyActive )
        {
            ...
            {
                NSLog(@"Video still waiting to be pre-cached for slot (%@)...", ad.adIdNumber);
            }
        }
        else
        {
    ...
    {
        NSLog(@"Begin refresh for nil slot. Clearing view...");
        [self clearView];
    }
}
...
- (void)playVideoIfInactive
{
    ...
    {
        NSLog(@"Attempting to play a video that's already playing");
    }
    else
    {
        NSLog(@"Video play requested...");
        ...
        {
            NSLog(@"Tracking video start for native ad (%@)", self.ad.adIdNumber);
...
{
    NSLog(@"Video finished playing for native ad (%@)", self.ad.adIdNumber);
...
{
    NSLog(@"Redirecting from Learn More button");
    [self handleClick];
}
...
- (void)didTapVideo:(UITapGestureRecognizer *)tapGesture
{
    ...
    {
        NSLog(@"Redirecting from video click");
        [self handleClick];
        [self setInactive];
    }
    else
...
{
    NSLog(@"Redirecting from ad image click");
    [self handleClick];
}
