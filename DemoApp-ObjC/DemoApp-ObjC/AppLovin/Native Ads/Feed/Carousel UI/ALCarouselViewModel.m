...
- (ALCarouselCardState *)cardStateAtNativeAdIndex:(NSUInteger)index
{
    ...
    {
        NSLog(@"Requested card state at native ad index %lu is out-of-bounds", index);
        return nil;
    }
    else
    {
        ...
        if ( cardState )
        {
            NSLog(@"Requested card state at native ad index %lu", index);
            return cardState;
        }
        else
        {
            NSLog(@"Requested card state at native ad index %lu does not exist yet. Creating new card state", index);
...
- (ALNativeAd *)nativeAdAtIndex:(NSUInteger)index
{
    ...
    {
        NSLog(@"Requested native ad index %lu", index);
        return [self.nativeAds objectAtIndex: index];
    }
    else
    {
        NSLog(@"Requested native ad index %lu is out of bounds", index);
        return nil;
    }
}
